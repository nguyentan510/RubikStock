...
Business review được điều phối tại [`D1_BUSINESS_ACCEPTANCE.md`](D1_BUSINESS_ACCEPTANCE.md).
...
| D1.4 | Outbound contract | `OUT-001..011`, shelf-life, FEFO override, reservation lifecycle | Multi-Lot/concurrent/short-pick scenarios |
| D1.5 | Quality và exception | `QLT-001..004`, `RET-001..005`, `DST-001..004`, approval matrix | Return/destruction/recall walkthrough |
| D1.6 | Delivery và planning boundary | `DEL-001..007`, `PLN-001..007` | Partial/failed delivery và advisory forecast review |
| D1.7 | Audit/security contract | `AUD-001..006`, `SEC-001..006` | Privileged-action, evidence-access và permission scenarios |
...
- [x] Không còn xung đột về inventory, Lot, UOM, reservation, return, destruction và delivery trong review scope.
- [x] State ownership và approval ownership được chấp nhận.
- [x] Conceptual data model hỗ trợ mọi scenario đã duyệt.
- [x] Các điểm cố ý defer được ghi rõ, không ngầm coi là accepted.
